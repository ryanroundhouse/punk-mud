...
describe('Upload Service Integration', () => {
  ...
  beforeEach(() => {
    ...
    app.post('/api/upload/mob', upload.single('image'), (req, res) => {
      // Check for mobId query parameter in the integration test
      if (!req.query.mobId) {
        return res.status(400).json({ error: 'Mob ID is required' });
      }
      // Construct the expected path including mobId
      const mobId = req.query.mobId;
      // Use the actual filename generated by the service (mocked here)
      const filename = req.file.filename || 'mock-filename.jpg'; 
      res.status(200).json({ 
        path: `/assets/mobs/${mobId}/${filename}`,
        filename: req.file.originalname 
      });
    });
  ...
  it('should handle API-style mob uploads', async () => {
    const testMobId = '605fe1a6f790a1001c27e8a1'; // Example valid ObjectId
    const response = await request(app)
      // Add mobId to the query string
      .post(`/api/upload/mob?mobId=${testMobId}`)
      .attach('image', Buffer.from('test'), 'mob-image.jpg');
      
    expect(response.status).toBe(200);
    expect(response.body).toHaveProperty('filename', 'test.jpg');
    
    // Verify the path includes the mobId and a filename
    expect(response.body.path).toContain(`/assets/mobs/${testMobId}/`);
    // Check that the path ends with a filename extension (e.g., .jpg)
    expect(response.body.path).toMatch(/\.\w+$/);
  });
